...
class ISBNet(nn.Module):
    def __init__(
        ...
        print(f"[DEBUG] ISBNet __init__: The ignore_label received is {ignore_label}")
    ...
    def forward_train(
        ...
        **kwargs
        ...
        # =================================================================
        # THE FIX: Define object points based on GROUND TRUTH labels, not
        # the model's own (initially random) predictions. This forces the
        # model to learn what the background is from the start.
        # =================================================================
        object_conditions = voxel_semantic_labels != self.ignore_label
        # =================================================================
        ...
        # Check if any scene in the batch has become empty after filtering.
        # If so, this can cause an indexing crash in the point aggregator.
        # We will return a placeholder loss for this problematic batch.
        # =================================================================
        #voxel_batch_idxs_ = voxel_batch_idxs[object_idxs]
        #counts = torch.bincount(voxel_batch_idxs_, minlength=batch_size)
        #if torch.any(counts == 0):
        #    loss_dict = {
        #        "placeholder_empty_batch": torch.tensor(0.0, requires_grad=True, device=semantic_labels.device, dtype=torch.float)
        #    }
        #    return self.parse_losses(loss_dict)
        # =================================================================
    ...
    def forward_head(
        ...
        for b in range(batch_size):
            ...
            for i in range(num_chunks):
                ...
                print(f"[DEBUG] Batch {b} | Chunk {i} | mask_feature_b: {mask_feature_b.shape}, "
                    f"weights_split[0]: {weights_split[0].shape}, biases_split[0]: {biases_split[0].shape}")
                ...
                print(f"[DEBUG] mask_logit chunk {i} output shape: {mask_logit_.shape}")
    ...
    def mask_heads_forward(
        ...
        for i, (w, b) in enumerate(zip(weights, biases)):
            # <<< MODIFICATION START: The debug block is now correctly placed INSIDE the loop. >>>
            print("====================== DEBUG INFO (Loop Start) ======================")
            print(f"[DEBUG] Loop iteration: {i}")
            print(f"[DEBUG] Shape of w (kernels): {w.shape}")
            print(f"[DEBUG] Shape of x (point features): {x.shape}")
            print(f"[DEBUG] Shape of b (biases): {b.shape}")
            print(f"[DEBUG] Dtype of w: {w.dtype}, Dtype of x: {x.dtype}")
            print("====================================================================")
            # <<< MODIFICATION END >>>
    ...
    def get_instance(
        ...
        for i in range(cls_final.shape[0]):
            ...
            elif self.dataset_name in ["stpls3d", "forinstance"]:
                pred["label_id"] = cls_final[i] + 1
            else:
