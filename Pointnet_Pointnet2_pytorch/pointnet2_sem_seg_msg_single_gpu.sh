...
# --- SLURM Directives ---
#SBATCH --job-name=pointnet2_msg_train_1gpu  # Specific name for single GPU training
#SBATCH --output=pointnet2_msg_train_job%j.log # Specific log file with Job ID (%j = Job ID)
#SBATCH --nodes=1                         # Run on a single node
#SBATCH --ntasks=1                        # Run a single task (your Python script)
#SBATCH --gres=gpu:1             # Number of GPU per nodes
#SBATCH --cpus-per-task=8                 # Request CPU cores (adjust based on DataLoader needs, >= num_workers)
#SBATCH --time=24:00:00                   # !!! ADJUST: Request sufficient walltime (e.g., 4 hours) !!!
#SBATCH --mem=32G                         # !!! ADJUST: Request sufficient memory (e.g., 32 GB) !!!
...
python $PROJECT_DIR/train_semseg.py \
    ...
    --data_path $DATA_DIR \
    --log_dir "pointnet2_msg_training_run_${SLURM_JOB_ID}" \
    --batch_size 64 \
    --epoch 32 \
    --learning_rate 0.001 \
    --npoint 1024 \
