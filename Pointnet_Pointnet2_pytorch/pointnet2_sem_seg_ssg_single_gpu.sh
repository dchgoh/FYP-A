...
# --- SLURM Directives ---
#SBATCH --job-name=pointnet2_ssg_train_1gpu  # Specific name for single GPU training
#SBATCH --output=pointnet2_ssg_train_job%j.log # Specific log file with Job ID (%j = Job ID)
...
python $PROJECT_DIR/train_semseg.py \
    ...
    --data_path $DATA_DIR \
    --log_dir "pointnet2_ssg_training_run_${SLURM_JOB_ID}" \
    --batch_size 64 \
    --epoch 32 \
    --learning_rate 0.001 \
    --npoint 1024 \
