...
# The new function now accepts a 'map_location' argument
def load_checkpoint(checkpoint, logger, model, optimizer=None, strict=False, map_location=None):
    if hasattr(model, "module"):
        model = model.module
        
    # We now use the flexible map_location provided by the caller script.
