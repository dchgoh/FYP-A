...
def calculate_tree_heights_adjusted(
    ...
    try:
        ...
        # Log for debugging ID field access
...
def calculate_tree_dbh(
    ...
    min_points_for_dbh_fit # Renamed for clarity
):
    tree_dbh_values = {}
    if not target_tree_ids:
...
if __name__ == "__main__":
    ...
    try:
        ...
        extracted_ids_list_str = sorted([str(tid) for tid in extracted_ids_set]) # Store as list of strings for JSON keys

        output_results["tree_ids"] = extracted_ids_list_str
        output_results["num_trees"] = len(extracted_ids_list_str)

        if extracted_ids_set: # Proceed only if IDs were found
            ...
            if midpoints_original_crs and transformer_to_wgs84:
                for tree_id_str, coords_dict in midpoints_original_crs.items():
                    if coords_dict: # Check if coords_dict is not None
                        try:
                    ...
                    elif SOURCE_EPSG_COORD_TRANSFORM == 4326 and coords_dict:
            ...
            if tree_dbhs_d1_meters_dict:
                for tree_id_str, dbh_m in tree_dbhs_d1_meters_dict.items():
                    if dbh_m is not None and not np.isnan(dbh_m):
                        temp_dbhs_d1_cm[tree_id_str] = round(dbh_m * 100, 2)
                    else:
            ...
            for tree_id_str_key in extracted_ids_list_str: # Use the string list for iteration
                ...
                volume = calculate_smalians_volume(d1_cm, d2_for_volume_cm, length_m)
                log_stderr("VolumeCalcOutput", f"Tree ID: {tree_id_str_key} -> Calculated Volume: {volume}")
                temp_volumes_m3[tree_id_str_key] = round(volume, 6) if volume is not None else None
            output_results["tree_volumes_m3"] = temp_volumes_m3
        else:
        ...
        print(json.dumps(output_results)) # Still print results gathered so far + errors
        sys.exit(1)
