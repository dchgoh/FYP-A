...
const CreateProjectModal = ({
    ...
    return (
        <Dialog open={createProjectModalOpen} onClose={handleCloseCreateProjectModal} disableEscapeKeyDown={isCreatingProject} PaperProps={{ sx: { backgroundColor: colors.grey[800] } }}>
            ...
            <DialogActions sx={{ p: theme.spacing(2, 3) }}><Button onClick={handleCloseCreateProjectModal} color="secondary" disabled={isCreatingProject}>Cancel</Button><Button onClick={handleCreateProject} color="primary" disabled={isCreatingProject || !newProjectName.trim() || !selectedDivisionIdForCreation} variant="contained">{isCreatingProject ? <CircularProgress size={24} color="inherit"/> : "Create"}</Button></DialogActions>
        </Dialog>
    );
};
